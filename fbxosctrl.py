...
class FbxConfiguration:
    ...
    def freebox_address(self):
        url = f"{self._addr_params['protocol']}://{self._addr_params['api_domain']}:{self._addr_params['port']}"
        return url
    ...
    def load(self, want_regapp):
        ...
        if self._reg_params is None:
            if not want_regapp:
                print(
                    "No registration params found in directory: {}".format(
                        self._conf_path
                    )
                )
                print(
    ...
    def has_registration_params(self):
        ...
        if (
            ...
            and self._reg_params.get("app_token") != ""
        ):
            return True
        else:
            return False
    ...
    def _fetch_fbx_mdns_info_via_mdns(self):
        ...
        try:
            ...
            info["https_available"] = (
                True if serv_info.properties[b"https_available"] == b"1" else False
            )
    ...
    def _load_addressing_params(self):
        ...
        if os.path.exists(self._addr_file):
            ...
            self._addr_params["protocol"] = (
                "https" if mdns_info["https_available"] else "http"
            )
...
class FbxResponse:
    ...
    def __init__(self, jsonresp):
        ...
        elif self._resp.get("success") not in (True, False):
            raise FbxException("Field success must be either true or false")

        if not self._resp["success"]:
...
class FbxHttp:
    ...
    def get(self, uri, timeout=None, no_login=False):
        ...
        if requests.codes.ok != r.status_code:
            raise FbxException(
                "GET error - http_status: {} {}".format(r.status_code, r.text)
            )
    ...
    def put(self, uri, data, timeout=None, no_login=False):
        ...
        if requests.codes.ok != r.status_code:
            raise FbxException(
                "PUT error - http_status: {} {}".format(r.status_code, r.text)
            )
    ...
    def post(self, uri, data={}, timeout=None, no_login=False):
        ...
        if requests.codes.ok != r.status_code:
            raise FbxException(
                "POST error - http_status: {} {}".format(r.status_code, r.text)
            )
    ...
    def _login(self):
        ...
        if not self._is_logged_in:
            ...
            log(f"challenge: {self._challenge}, apptoken: {app_token}")
            # Hashing token with key
            password = hmac.new(
                app_token.encode(), self._challenge.encode(), "sha1"
            ).hexdigest()
            uri = "/login/session/"
            payload = {
                "app_id": self._conf.app_desc.get("app_id"),
                "password": password,
            }
            # post it
...
class FbxServiceAuth(FbxService):
    ...
    def get_registration_status(self):
        ...
        if self._conf.has_registration_params():
            uri = f"/login/authorize/{self._conf.reg_params.get('track_id')}"
    ...
    def get_registration_status_diagnostic(self):
        ...
        if "granted" == status:
            ...
            print(
                ...
                + " on Freebox Server lcd/touchpad (track_id = {}).".format(track_id)
            )
        elif "unknown" == status:
            print(
            ...
            print(
                "This app has been denied by user on Freebox Server (track_id = {}).".format(
            ...
            print(
                ...
                + " to Freebox Server to get a new app_id (current track_id = {}).".format(
                    track_id
                )
            )
        else:
...
class FbxServiceSystem(FbxService):
    ...
    def get_system_info(self):
        ...
        print("Server info:")
        model_info = resp.result.get("model_info")
        if model_info and "pretty_name" in model_info:
            model_info = model_info["pretty_name"]
        print(f" - Model:     {model_info}")
        print(f" - MAC:       {resp.result.get('mac')}")
        print(f" - Firmware:  {resp.result.get('firmware_version')}")
        print(f" - Uptime:    {resp.result.get('uptime')}")
        print(" - Sensors:")
        for sensor in resp.result.get("sensors"):
            unit = "°C" if sensor["id"].startswith("temp_") else ""
            print(f"   - {sensor.get('name'):20s}: {sensor.get('value')}{unit}")
        return True
...
class FbxServiceConnection(FbxService):
    ...
    def get_line_ethernet_info(self):
        ...
        print("Ethernet info:")
        print(" - Info:")
        print("   - IPv4:   {}".format(resp.result.get("ipv4")))
        print("   - IPv6:   {}".format(resp.result.get("ipv6")))
        print("   - Media:  {}".format(resp.result.get("media")))
        print("   - State:  {}".format(resp.result.get("state")))
        print(" - Down:")
        print(
            "   - Bandwidth:     {}".format(
                FbxServiceConnection.rate_to_human_readable(
                    resp.result.get("bandwidth_down")
                )
            )
        )
        print(
        ...
        print(
            "   - Bandwidth:     {}".format(
                FbxServiceConnection.rate_to_human_readable(
                    resp.result.get("bandwidth_up")
                )
            )
        )
        print(
            "   - Current rate:  {}".format(
                FbxServiceConnection.rate_to_human_readable(resp.result.get("rate_up"))
            )
        )
        return True
    ...
    def _get_xdsl_info(self):
        ...
        print("xDSL info:")
        print(" - Status:")
        for k, v in resp.result.get("status").items():
            print("   - {:13} {}".format(k + ":", v))
        down = resp.result.get("down")
        print(" - Down:")
        print(
            "   - Max Rate:     {}".format(
                FbxServiceConnection.rate_to_human_readable(down["rate"] * 1000)
            )
        )
        print("   - Attenuation:  {} dB".format(down["attn_10"] / 10))
        print("   - Noise magin:  {} dB".format(down["snr_10"] / 10))
        up = resp.result.get("up")
        print(" - Up:")
        print(
            "   - Max Rate:     {}".format(
                FbxServiceConnection.rate_to_human_readable(up["rate"] * 1000)
            )
        )
    ...
    def _get_ftth_info(self):
        ...
        if (
            resp.result.get("has_sfp") is not True
            and resp.result.get("sfp_present") is not True
        ):
        ...
        print("FTTH info:")
        print(" - SPF Module:")
        print(f"   - Model:     {resp.result.get('sfp_model')}")
        print(f"   - Vendor     {resp.result.get('sfp_vendor')}")
        print(f"   - Serial:    {resp.result.get('sfp_serial')}")
        print(" - Status:")
        print(f"   - Signal:    {resp.result.get('sfp_has_signal')}")
        print(f"   - Alim:      {resp.result.get('sfp_alim_ok')}".format())
        # print('   - Powered:   {}'.format(resp.result.get("sfp_has_power_report")))
        if resp.result.get("link"):
            print(f"   - Link:      {resp.result['link']}")
        else:
            print(" - Link:")
            print(f"   - Tx:  {resp.result.get('sfp_pwr_tx') / 100:.2f} dB")
            print(f"   - Rx:  {resp.result.get('sfp_pwr_rx') / 100:.2f} dB")
        return True
        ...
        def display_port_forwarding_entry(count, pforwarding):
            ...
            print(
                data.format(
                    count,
                    pforwarding.get("id"),
                    pforwarding.get("enabled"),
                    pforwarding.get("hostname"),
                    pforwarding.get("comment"),
                    pforwarding.get("lan_port"),
                    pforwarding.get("wan_port_start"),
                    pforwarding.get("wan_port_end"),
                    pforwarding.get("src_ip"),
                    pforwarding.get("lan_ip"),
                    pforwarding.get("ip_proto"),
                )
            )
